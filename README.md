# AITCH Technologies LLC

Official website for AITCH Technologies LLC, the patent holding and licensing division of AITCH Holdings LLC.

## Overview

AITCH Technologies LLC specializes in developing and licensing innovative patents in sustainable construction, agriculture, and energy sectors.

## Patent Portfolio

The company manages ten patent-holding entities (all Delaware LLCs):

### Construction
| Entity | Patent Title | USPTO App # | Status |
|--------|-------------|-------------|--------|
| TerraGrid Systems LLC | Modular Interlocking Infrastructure Block | 19/306,897 | Pre-exam |
| BioTile Systems LLC | Biofilter Tile with Embedded Channels | 19/305,544 | Pre-exam |
| SmartBuild Solutions LLC | Modular Wall Block System | 19/312,003 | Docketed |
| HydroShingle Systems LLC | Rainwater-Capturing Roofing Shingle | 19/311,898 | Docketed |

### Agriculture
| Entity | Patent Title | USPTO App # | Status |
|--------|-------------|-------------|--------|
| AgriFlow Systems LLC | Passive Irrigation Membrane System | 19/305,413 | Docketed |
| EcoInfuse Systems LLC | TBD | Pending | Pending |
| SkyRoot Systems LLC | Vertical Root-Zone Air Purifier | Pending | Pending |

### Energy
| Entity | Patent Title | USPTO App # | Status |
|--------|-------------|-------------|--------|
| ClimateVent Systems LLC | Climate-Adaptive Ventilation System | 19/306,944 | Pre-exam |
| Verdant Appliance Co. LLC | Greywater Recycling Dishwasher | 19/312,069 | Pre-exam |
| UrbanRay Systems LLC | Solar Street Marker System | 19/311,959 | Docketed |

## Corporate Structure

```
AITCH Holdings LLC (Parent Company)
    └── AITCH Technologies LLC (Patent Holding Division)
            ├── AgriFlow Systems LLC
            ├── BioTile Systems LLC
            ├── ClimateVent Systems LLC
            ├── EcoInfuse Systems LLC
            ├── HydroShingle Systems LLC
            ├── SkyRoot Systems LLC
            ├── SmartBuild Solutions LLC
            ├── TerraGrid Systems LLC
            ├── UrbanRay Systems LLC
            └── Verdant Appliance Co. LLC
```

## Technology Stack

- HTML5
- CSS3 (Custom Properties, Flexbox, Grid)
- Vanilla JavaScript
- Lucide Icons
- Google Fonts (Sora, DM Sans)

## Development

This is a static website. To run locally, simply open `index.html` in a web browser or use a local server:

```bash
# Using Python
python -m http.server 8000

# Using Node.js
npx serve
```

## Contact

- Email: licensing@aitchtechnologies.com
- Address: 251 Little Falls Drive, Wilmington, DE 19808

## License

Copyright 2025 AITCH Technologies LLC. All rights reserved.
